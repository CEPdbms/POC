...
os.environ['REPLICATE_API_TOKEN']='r8_9nZfmJcc3oqxkLFWsM4BIToRqoKiVRR3Vi642'
os.environ['OPENAI_API_KEY']='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'
...
def conversation_chat(query, chain, history):
    result = chain[0]({"question": query, "chat_history": history})
   # history.append((query, result["answer"]))
    re=chain[1].as_retriever(search_kwargs={"k": 2})
    eval_result_c1=evaluation_on_crit(query,result,re,criteria="correctness")
    eval_result_c2=evaluation_on_crit(query,result,re,criteria="relevance")
    eval_result_c3=evaluation_on_crit(query,result,re,criteria = {
            "hallucination": ("Does this submission contain information"
    " not related to the input or reference?")})
    data = {
    "question": query,
    #"answer": result["answer"]["answer"],
    "correctness": eval_result_c1["score"],
    "reasoning_correctness": eval_result_c1["reasoning"],
    "relevance": eval_result_c2["score"],
    "reasoning_relevance_": eval_result_c2["reasoning"],
    "hallucination": eval_result_c3["score"],
     "reasoning_hallucination": eval_result_c3["reasoning"]
    }
...
For questions unrelated to Universal Robots oem products, reply with 'I don't have knowledge about this. I am a personal Bot assistant providing Universal Robots oem product-related assistance.'
...
def create_conversational_chain(vector_store,llm,select_model):
    #load_dotenv()
    if select_model=='Llama 2 7b' or select_model=='Llama 2 13b' or select_model=='Llama 2 70b':
       prompt = set_custom_prompt_llama()
    else:
...
def main():
    ...
    model_kwargs={"temperature": 0.02, "max_output_tokens": 700, "top_p": 1})
        ...
        llm= GooglePalm(model="chat-bison@001", max_output_tokens=700, temperature=0.02,top_p=1)
